...
import { DeleteSaleInvoiceCreditorSubmit } from '../../../services/CreditorScreen/finishCreditor/DeleteSubmitCreditor'
import { ToastAndroid } from 'react-native';
export const DeleteSubmitCreditor = (props: any) => {
    const { profileInfo } = useSelector((state: any) => ({
        profileInfo: state?.auth?.profileInfo,
    }));
    const [ErrorDescription, setErrorDescription] = useState('');
    const onPressDeleteSubmit = async (VoucherSubmitID: any) => {
        let dataResult = await DeleteSaleInvoiceCreditorSubmit(profileInfo.UserID,VoucherSubmitID);
        if (dataResult != null) {

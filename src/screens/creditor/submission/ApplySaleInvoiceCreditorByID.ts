...
import { ApplySaleInvoiceCreditorByID } from '../../../services/CreditorScreen/submissionCreditor/ApplySaleInvoiceCreditorByID';

export const applySaleInvoiceCreditorByID = (props: any) => {
  ...
  const onPressSubmiss = async (VoucherSubmit : VoucherSubmit, VoucherSubmitDetai:Array<VoucherSubmitDetail>) => {
    let dataResult = await ApplySaleInvoiceCreditorByID(profileInfo.UserID, VoucherSubmit, VoucherSubmitDetai);
    if (dataResult != null) {

...
import styles from './BackgroundDetailCreditorScreen.style';
import { NavigationState, useRoute } from '@react-navigation/native';
import LinearGradient from 'react-native-linear-gradient';
import { mainColors } from '../../../constants';
interface BackgroundDetailCreditorScreen {
  title: string;
...
const Item = ({ item, onPress, borderColor }) => (
  <TouchableOpacity style={[borderColor, styles.BackgroudImage]} onPress={() => onPress(item)}>
    <Image source={item.urlImage} style={styles.urlImage} />
  </TouchableOpacity>
);
export const BackgroundDetailCreditorScreen = (props: BackgroundDetailCreditorScreen) => {
  const { children, containerStyle, title, navigation, detail } = props;
  const [selectedId, setSelectedId] = useState(null);
  const goBack = () => {
    navigation?.navigate('Creditor');
  };

  const functionList = [
    {
      name: '1. Chờ nhận',
      purposeName: 'WaitingCreditor',
      urlImage: WAITINGINVOICEICONLIST,
    },
    {
      name: '2. Xử lý',
      purposeName: 'HandleCreditor',
      urlImage: exchanged,
    },
    {
      name: '3. Nộp phiếu',
      purposeName: 'SubmissionCreditor',
      urlImage: FILE,
    },
    {
      name: '4. Hoàn tất',
      purposeName: 'FinishCreditor',

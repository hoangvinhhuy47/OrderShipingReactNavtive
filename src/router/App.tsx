...
const App = (props: any) => {
  const {GuidID, isLogin, UrlString, isLoading, splashLoad, splashLoadSecondLogin } = useSelector(
    (state: any) => ({
      ...
      splashLoadSecondLogin: state.system.splashLoadSecondLogin,
    })
  );
  const { modal } = props;
  ...
  console.log({splashLoad, splashLoadSecondLogin, isLogin})
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <NavigationContainer>
        <Stack.Navigator screenOptions={{ headerShown: false }}>
          {(splashLoad || splashLoadSecondLogin) && <Stack.Screen name='SplashScreen' component={SplashScreen} />}
          {!GuidID ? (
